---
name: upload-cyberpi
description: Upload a MicroPython file to the CyberPi via the mBlock web IDE using browser automation (chrome-devtools MCP). Use when the user asks to upload/flash/test code on the CyberPi/mBot2.
---
...
In opencode this skill runs with the `chrome-devtools` MCP server (enabled
in the global config). The equivalent `claude-in-chrome` tool names from
the original Claude skill do not exist here — use the `chrome-devtools_*`
tools named in each step below.
...
- **Never simulate keystrokes to enter code into the editor.** Set the
  ...
  `monaco.editor.getModels()[0].setValue(code)` through `evaluate_script`.
  Keystroke typing risks the editor's auto-indent/auto-bracket
...
3. **Get/create a browser tab on `ide.mblock.cc`.**
   - Call `chrome-devtools_list_pages`. If an existing page is already on
     `ide.mblock.cc`, ask the user whether it's the one they've already
     connected before reusing it (select it with
     `chrome-devtools_select_page`). Otherwise open one with
     `chrome-devtools_new_page` and navigate to `https://ide.mblock.cc/`
     with `chrome-devtools_navigate_page`.
4. **Confirm the device is connected, in Upload mode.** Take a screenshot
   with `chrome-devtools_take_screenshot`. Look for the device panel
   showing a connected indicator (green dot / "Device connected") and the
   "Upload" toggle active (not "Live").
   - **If not connected: stop and ask the user to connect it themselves.**
     ...
     becoming unresponsive, consistent with a stuck native dialog). Do not
     click "Bluetooth" or "Serial" and then wait for a dialog yourself;
     ask the user to do it and tell you when it's connected.
   - If connected but in "Live" mode, click "Upload" (a confirmation dialog
     may appear — click through it) using `chrome-devtools_click` (take a
     fresh `chrome-devtools_take_snapshot` first to get the element uid).
5. **Switch to the Python code tab** if the Blocks view is showing (click
   "Python" in the top-right of the editor pane via `chrome-devtools_click`
   after a `chrome-devtools_take_snapshot`).
6. **Set the editor content** via `chrome-devtools_evaluate_script`:
   ```js
...
8. **Click "Upload Code"** via `chrome-devtools_click`. Wait a few seconds,
   then screenshot. Watch for:
